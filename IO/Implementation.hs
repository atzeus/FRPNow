...
module IO.Implementation (TimeStamp,Event, never, seqE, Behaviour, switch, seqB, whenJust, Now, liftB, act, plan, runNow) where
...
liftB b = Now $ prim (LiftB b)
act :: IO a -> Now s (Event s a)
act m   = Now $ prim (Act m)

plan p  = Now $ prim (Plan p)
...
runNow (Now p) = 
    do f <- newFlag
       hPutStr stderr "Starting"
       evalStateT 
         (do e <- runPlanIO' p ; printErr "Done init" ; runTimeSteps e)  
         (initState f)
...
runTimeSteps e = loop where
  loop = do printErr "Starting next time"
            ev <- getEv e 
            case ev of
                Just a -> return a
                Nothing -> 
...
runPlanIO' t = case viewTermM t of
    Return a -> return a
    p :>>= f -> printErr "Handling prim" >> handlePrim p >>= runPlanIO' . f 
    where handlePrim :: PlanIOPrim s a -> PIOM s a
          handlePrim (LiftB b) = printErr"LiftB" >> getBehaviour  b
          handlePrim (Fix f)   = printErr "Fix" >> mfix (runPlanIO' . getNow . f)
          handlePrim (Act m)   = printErr "Act" >> startIO m
          handlePrim (Plan e)  = printErr "Plan" >> plan' e

plan' :: Event s (Now s a) -> PIOM s (Event s a)
plan' e = do r <- lift $ newSIORef
             n <- getEv e
             case n of
                Just (Now a)  -> do v <- runPlanIO' a; lift (putSIORef r v)
